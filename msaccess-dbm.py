...
"""
# To get constant values, open Access, make sure ADODB and ADOX are
# references,
...
def encoding(value):
	if isinstance(value, str):
		...
		if len(value) == 0:
			...
			return value.encode(
				"mbcs")  # mbcs is a Windows, locale-specific encoding
...
class Recordset():
	...
	def __init__(self, connAccess, SQL_statement, dict):
		self.rs = win32com.client.Dispatch(r'ADODB.Recordset')
		self.rs.CursorLocation = 3  # uses client - makes it possible to use
	...
	def paged(self, pagesize=128):
		...
		try:
			...
			# field_names = [self.encoding(field.Name) for field in 
			# recordset.Fields]
			ok = True
			while ok:
