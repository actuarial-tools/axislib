...
class query:
	...
	def __init__(self, s):
		...
		for t in tokenize.generate_tokens(
				open(s.gi_frame.f_code.co_filename).readline):
...
if __name__ == '__main__':
	...
	#from db_iterator import query, \
	#	Table

	conn = sqlite3.connect('planes')
	query.cursor = conn.cursor()
	...
	# this request returns a list instances of Record with the attribute 
	# c_country (c.country with the . replaced by _)
	print(country for country in query(c.country for c in countries if 
	                                   c.continent == 'Europe'))
